...
from models import MODEL_DICT, RetrieverEncoder
...
save_path = "result/0922_selfregularization"
gpu = "cuda:0" ##########
...
def knowledge_distillation_loss(logits, teacher_logits):
    ...
    teacher_logits=torch.zeros(batch_size,batch_size)
    #print(pred) # torch.Size([96, 96])
    target = torch.arange(batch_size).to(device)  
    ...
    for i in range (batch_size):
        for j in range(batch_size):
            if i==j:
                teacher_logits[i][j]=0.9
            else: 
                teacher_logits[i][j]=1/(batch_size-1)*0.1
    ...
    #distillation_loss =torch.nn.functional.kl_div(torch.nn.functional.log_softmax(logits/T, dim=1), torch.nn.functional.softmax(teacher_logits/T, dim=1), reduction='batchmean') * (T * T)
    distillation_loss =torch.nn.functional.kl_div(torch.nn.functional.log_softmax(logits, dim=1), torch.nn.functional.softmax(teacher_logits, dim=1), reduction='batchmean') * (T * T)
...
teacher_model = MODEL_DICT[model_name]()
teacher_model.load_state_dict(torch.load("/home/nlplab/hdd1/yoo/KorDPR_retriever/result/0201_base/40.model",map_location='cuda:0'))
teacher_model.to(device)

model = MODEL_DICT[model_name]()
model.load_state_dict(torch.load("/home/nlplab/hdd1/yoo/KorDPR_retriever/result/0201_base/40.model",map_location='cuda:0'))
model.to(device)
...
def train():
    ...
    for e in range(epoch):
        for step, batch in enumerate(tqdm(train_dataloader, desc="Epoch {}".format(str(e)))):
            ...
            # teacher_q_emb = teacher_model(q, q_mask, "query")  
            # teacher_p_emb = teacher_model(p, p_mask, "passage")  
            
            # teacher_pred = torch.matmul(teacher_q_emb, teacher_p_emb.T) # bs*bs 
